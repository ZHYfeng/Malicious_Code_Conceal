...
#define pardo(i, first, last, inc) SWARM_pardo((i), (first), (last), (inc))
...
int _SWARM_MULTICORE_reduce_i(_SWARM_MULTICORE_reduce_i_t nbarrier, int val,
                              reduce_t op);
...
long _SWARM_MULTICORE_reduce_l(_SWARM_MULTICORE_reduce_l_t nbarrier, long val,
                               reduce_t op);
...
double _SWARM_MULTICORE_reduce_d(_SWARM_MULTICORE_reduce_d_t nbarrier,
                                 double val, reduce_t op);
...
int _SWARM_MULTICORE_scan_i(_SWARM_MULTICORE_scan_i_t nbarrier, int val,
                            reduce_t op, int th_index);
...
long _SWARM_MULTICORE_scan_l(_SWARM_MULTICORE_scan_l_t nbarrier, long val,
                             reduce_t op, int th_index);
...
double _SWARM_MULTICORE_scan_d(_SWARM_MULTICORE_scan_d_t nbarrier, double val,
                               reduce_t op, int th_index);
...
void _SWARM_MULTICORE_spin_barrier_destroy(
    _SWARM_MULTICORE_spin_barrier_t sbarrier);
void _SWARM_MULTICORE_spin_barrier_wait(
    _SWARM_MULTICORE_spin_barrier_t sbarrier);
...
void radixsort_swarm_s3(long q, int *lKeys, int *lSorted, THREADED);
void radixsort_swarm_s2(long q, int *lKeys, int *lSorted, THREADED);
void radixsort20_swarm_s1(long q, int *lKeys, int *lSorted, THREADED);
void radixsort20_swarm_s2(long q, int *lKeys, int *lSorted, THREADED);

void radixsort_check(long q, int *lSorted);
...
{
  register int j, k, last, temp, offset;

  int *myHisto, *mhp, *mps, *psHisto, *allHisto;
...
void create_input_random_swarm(int myN, int *x, THREADED) {
  create_seq_random_swarm(317 * (MYTHREAD + 17), _NAS_MULT, myN, x, TH);
}
...
void create_input_nas_swarm(int n, int *x, THREADED) {
  ...
  create_seq_swarm(find_my_seed(mynum, thtot, (n >> 2),
...
int SWARM_mutex_init(SWARM_mutex_t **mutex, const SWARM_mutexattr_t *attr,
  ...
  *mutex = (SWARM_mutex_t *)SWARM_malloc(sizeof(SWARM_mutex_t), TH);
  on_one_thread { r = pthread_mutex_init(*mutex, attr); }
...
static void SWARM_Barrier_tree_init(void) {
  int i;
...
static void SWARM_Barrier_tree_destroy(void) { return; }

static void SWARM_Barrier_tree_up(THREADED) {
  ...
  if (MYTHREAD == 0) {
    if (THREADS > 2) {
      while (up_buf[NOSHARE(myidx)][0] == 0 || up_buf[NOSHARE(myidx)][1] == 0)
        ;
    } else if (THREADS == 2) {
      while (up_buf[NOSHARE(myidx)][1] == 0)
        ;
    }
  } else if (odd_child >= THREADS)
    up_buf[NOSHARE(parent)][parity]++;
  else if (even_child >= THREADS) {
    while (up_buf[NOSHARE(myidx)][1] == 0)
      ;
    up_buf[NOSHARE(parent)][parity]++;
  } else {
...
static void SWARM_Barrier_tree_down(THREADED) {
...
int SWARM_Bcast_i(int myval, THREADED) {
  ...
  on_one_thread { _SWARM_bcast_i = myval; }
...
long SWARM_Bcast_l(long myval, THREADED) {
  ...
  on_one_thread { _SWARM_bcast_l = myval; }
...
double SWARM_Bcast_d(double myval, THREADED) {
  ...
  on_one_thread { _SWARM_bcast_d = myval; }
...
char SWARM_Bcast_c(char myval, THREADED) {
  ...
  on_one_thread { _SWARM_bcast_c = myval; }
...
int *SWARM_Bcast_ip(int *myval, THREADED) {
  ...
  on_one_thread { _SWARM_bcast_ip = myval; }
...
long *SWARM_Bcast_lp(long *myval, THREADED) {
  ...
  on_one_thread { _SWARM_bcast_lp = myval; }
...
double *SWARM_Bcast_dp(double *myval, THREADED) {
  ...
  on_one_thread { _SWARM_bcast_dp = myval; }
...
static int SWARM_read_arguments(int argc, char **argv) {
  ...
  if (argv[0] == NULL)
    SWARM_error(__LINE__, "SWARM_read_arguments", "invalid argument array");
  ...
  while ((c = getopt(argc, argv, "ht:o:")) != -1) {
    ...
    case 'o':
      ...
      if ((SWARM_outfile = fopen(SWARM_outfilename, "w")) == NULL)
        SWARM_error(__LINE__, "read_arguments", "unable to open outfile %s",
                    SWARM_outfilename);
      break;
    ...
    default:
      SWARM_error(__LINE__, "read_arguments", "`%c': unrecognized argument", c);
...
static void SWARM_get_args(int *argc, char ***argv) {
  int numarg = *argc;
  int done = 0;
  char *s, **argvv = *argv;
...
void SWARM_Init(int *argc, char ***argv) {
  ...
  spawn_thread = (pthread_t *)malloc(NOSHARE(THREADS) * sizeof(pthread_t));
  assert_malloc(spawn_thread);
  uthread_info =
      (uthread_info_t *)malloc(NOSHARE(THREADS) * sizeof(uthread_info_t));
...
_SWARM_MULTICORE_barrier_t _SWARM_MULTICORE_barrier_init(int n_clients) {
  _SWARM_MULTICORE_barrier_t nbarrier = (_SWARM_MULTICORE_barrier_t)malloc(
      sizeof(struct _SWARM_MULTICORE_barrier));
...
_SWARM_MULTICORE_reduce_i_t _SWARM_MULTICORE_reduce_init_i(int n_clients) {
  _SWARM_MULTICORE_reduce_i_t nbarrier = (_SWARM_MULTICORE_reduce_i_t)malloc(
      sizeof(struct _SWARM_MULTICORE_reduce_i_s));
  assert_malloc(nbarrier);
...
_SWARM_MULTICORE_reduce_l_t _SWARM_MULTICORE_reduce_init_l(int n_clients) {
  _SWARM_MULTICORE_reduce_l_t nbarrier = (_SWARM_MULTICORE_reduce_l_t)malloc(
      sizeof(struct _SWARM_MULTICORE_reduce_l_s));
  assert_malloc(nbarrier);
...
_SWARM_MULTICORE_reduce_d_t _SWARM_MULTICORE_reduce_init_d(int n_clients) {
  _SWARM_MULTICORE_reduce_d_t nbarrier = (_SWARM_MULTICORE_reduce_d_t)malloc(
      sizeof(struct _SWARM_MULTICORE_reduce_d_s));
  assert_malloc(nbarrier);
...
_SWARM_MULTICORE_scan_i_t _SWARM_MULTICORE_scan_init_i(int n_clients) {
  _SWARM_MULTICORE_scan_i_t nbarrier = (_SWARM_MULTICORE_scan_i_t)malloc(
      sizeof(struct _SWARM_MULTICORE_scan_i_s));
  assert_malloc(nbarrier);
...
_SWARM_MULTICORE_scan_l_t _SWARM_MULTICORE_scan_init_l(int n_clients) {
  _SWARM_MULTICORE_scan_l_t nbarrier = (_SWARM_MULTICORE_scan_l_t)malloc(
      sizeof(struct _SWARM_MULTICORE_scan_l_s));
  assert_malloc(nbarrier);
...
_SWARM_MULTICORE_scan_d_t _SWARM_MULTICORE_scan_init_d(int n_clients) {
  _SWARM_MULTICORE_scan_d_t nbarrier = (_SWARM_MULTICORE_scan_d_t)malloc(
      sizeof(struct _SWARM_MULTICORE_scan_d_s));
  assert_malloc(nbarrier);
...
_SWARM_MULTICORE_spin_barrier_init(int n_clients) {
  _SWARM_MULTICORE_spin_barrier_t sbarrier =
      (_SWARM_MULTICORE_spin_barrier_t)malloc(
          sizeof(struct _SWARM_MULTICORE_spin_barrier));
