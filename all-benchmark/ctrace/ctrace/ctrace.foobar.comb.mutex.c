...
/* ----------------------------------------------------------------------
   ...
   desc: enable tracing 
   --------------------------------------------------------------------*/
#define TRC_ENABLE() _trc = TRC_ENABLED
...
/* ----------------------------------------------------------------------
   ...
   --------------------------------------------------------------------*/
#define TRC_DISABLE() _trc = TRC_DISABLED
...
/* ----------------------------------------------------------------------
   ...
   --------------------------------------------------------------------*/
#define TRC_ENTER(a,b,c) trc_trace(TRCA_ENTER, a, b, __FILE__, __LINE__, __FUNCTION__, NULL, trc_varargs c)
...
/* ----------------------------------------------------------------------
   ...
   --------------------------------------------------------------------*/
#define TRC_PRINT(a,b,c) trc_trace(TRCA_PRINT, a, b, __FILE__, __LINE__, __FUNCTION__, NULL,trc_varargs c)
...
static pthread_mutex_t _startclient;		/* dont start client until server inited */
...
static pthread_mutex_t _hashsem;
...
#else
#define HASH_WRITE_ENTER() pthread_mutex_lock(&_hashsem)
#endif

#ifdef ERR4
#define HASH_WRITE_EXIT() ;
#else
#define HASH_WRITE_EXIT() pthread_mutex_unlock(&_hashsem)
#endif
...
int trc_init(cchar_t *file, tenable_t trc, ton_t on, tlevel_t level, uint_t umax, tserver_t server)
{
	...
	pthread_mutex_init(&_hashsem, NULL);
	pthread_mutex_init(&_startclient, NULL);

	/* run trace as separate thread */
	if(_server == TRC_SERV_ON){
		pthread_mutex_lock(&_startclient);
...
void trc_end()
{
	...
	pthread_mutex_destroy(&_hashsem);
	pthread_mutex_destroy(&_startclient);
...
void *trc_start_server(void)
{
	...
	pthread_mutex_unlock(&_startclient);
